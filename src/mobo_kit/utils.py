...
def _read_csv_with_fallback(path: str) -> pd.DataFrame:
    """Read CSV with common encoding fallbacks for lab-exported files."""
    ...
    raise UnicodeDecodeError(
        last_exc.encoding if last_exc else "unknown",
        last_exc.object if last_exc else b"",
        last_exc.start if last_exc else 0,
        last_exc.end if last_exc else 0,
        (
            f"Failed to decode CSV '{path}'. Tried encodings: "
            + ", ".join(encodings)
        ),
    )
...
def split_XY(df: pd.DataFrame, design: Design, config: dict) -> Tuple[np.ndarray, np.ndarray]:
    ...
    # Extract data rows (skip the first 5 rows: units/start/stop/step + empty row)
    # Note: pd.read_csv already consumed row 1 (variable names) as column headers,
    # so df.iloc[0..3] = units/start/stop/step, df.iloc[4] = empty, df.iloc[5+] = data.
...
def csv_to_config(csv_path: str, output_path: str = None) -> str:
    ...
    """
    # Read CSV file with encoding fallback support
