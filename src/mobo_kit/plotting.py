...
def _validate_corr_method(method: str) -> CorrMethod:
    m = method.lower()
    if m not in ("pearson", "spearman"):
        raise ValueError(f"method must be 'pearson' or 'spearman', got {method!r}")
    return cast(CorrMethod, m)
...
def plot_correlation_heatmap(
    ...
    """
    Create a correlation heatmap for input features (Pearson or Spearman).
    ...
    method = _validate_corr_method(method)
    ...
    # Set title
    if title is None:
        title = f"{method.capitalize()} Correlation Matrix ({len(columns)} features)"
...
def plot_correlation_heatmap_full(
    X: pd.DataFrame,
    Y: pd.DataFrame,
    ...
    """
    Cross-correlation heatmap: rows = features (X), columns = targets (Y).

    Rows are aligned by index when possible; otherwise both frames are reindexed
    with a default RangeIndex. Column names must be unique across X and Y.

    Args:
        X: Feature DataFrame from split_XY() or similar.
        Y: Target/objective DataFrame, same number of rows as X.
        x_columns: Subset of feature columns (default: all columns in X).
        y_columns: Subset of target columns (default: all columns in Y).
        method: ``'pearson'`` (linear) or ``'spearman'`` (rank/monotonic).
        figsize: Figure size ``(width, height)``; default ``(n_y, n_x)``.
        fontsize: Font size for labels and title.
        cmap: Colormap for the heatmap.
        annot: Whether to show correlation values on the heatmap.
        fmt: Format string for correlation values.
    ...
    Returns:
        Tuple of (figure, correlation_matrix) with shape ``(n_features, n_targets)``.

    Example:
        >>> X_df, Y_df = split_XY(df, design, config)
        >>> fig, corr = plot_correlation_heatmap_full(
        ...     X_df, Y_df,
        ...     title="Features vs objectives",
        ...     save="results/corr_full.png",
        ...     show_plot=False,
        ... )
    """
    if len(X) != len(Y):
    ...
    X_sub = X if x_columns is None else X[list(x_columns)]
    Y_sub = Y if y_columns is None else Y[list(y_columns)]

    overlap = set(X_sub.columns) & set(Y_sub.columns)
    if overlap:
        raise ValueError(f"Duplicate column names between X and Y: {sorted(overlap)}")

    if X_sub.index.equals(Y_sub.index):
        combined = pd.concat([X_sub, Y_sub], axis=1)
    else:
        combined = pd.concat(
            [X_sub.reset_index(drop=True), Y_sub.reset_index(drop=True)],
            axis=1,
        )

    method = _validate_corr_method(method)
    x_names = list(X_sub.columns)
    y_names = list(Y_sub.columns)
    corr_matrix = combined.corr(method=method).loc[x_names, y_names]

    n_x, n_y = len(x_names), len(y_names)
    if figsize is None:
        figsize = (n_y, n_x)
    ...
    if title is None:
        title = f"{method.capitalize()} Correlation: {n_x} features × {n_y} targets"
    ax.set_title(title, fontsize=fontsize)
    ...
    return fig, corr_matrix.to_numpy()
...
def plot_shap(
    ...
    if X_eval.ndim != 2:
        raise ValueError(f"train_X must be a 2D array, got shape {X_eval.shape}")

    n_samples, n_features = X_eval.shape
    feature_names = list(getattr(design, "names", []))
    if len(feature_names) != n_features:
        feature_names = [f"x{i}" for i in range(n_features)]

    if not hasattr(model, "models"):
        raise ValueError("model must expose .models (e.g., a ModelListGP).")
    M = len(model.models)

    if objective_names is None:
        objective_names = [f"obj{i}" for i in range(M)]
    elif len(objective_names) != M:
        raise ValueError(f"objective_names length {len(objective_names)} != number of outputs {M}")

    def _coerce_shap_to_2d(shap_values_obj) -> np.ndarray:
        """Normalize SHAP outputs across SHAP versions into (n_samples, n_features)."""
        vals = shap_values_obj

        if hasattr(vals, "values"):
            vals = vals.values
        if isinstance(vals, list):
            vals = vals[0] if len(vals) else np.empty((0, n_features), dtype=float)
        ...
        # - (N, D)               : desired
        # - (N, D, 1) or (1, N, D)
        # - (D,)                 : single-sample output
        if vals.ndim == 1:
            vals = vals.reshape(1, -1)
        elif vals.ndim == 3:
            # Favor first output axis for scalar objective explainers.
            if vals.shape[-1] == 1:
                vals = vals[..., 0]
            elif vals.shape[0] == 1:
                vals = vals[0]
        ...
        # Try to align rows to N; transpose if needed.
        if vals.shape[0] != n_samples and vals.shape[1] == n_samples:
            vals = vals.T

        # Harmonize feature dimension to D.
        if vals.shape[1] < n_features:
            pad = np.full((vals.shape[0], n_features - vals.shape[1]), np.nan, dtype=float)
            vals = np.hstack([vals, pad])
        elif vals.shape[1] > n_features:
            vals = vals[:, :n_features]

        return vals

    # Small background set helps Kernel SHAP; k-means is robust.
    k_bg = max(1, min(30, n_samples))
    X_bg = shap.kmeans(X_eval, k=k_bg)
    ...
    for i, obj_name in enumerate(objective_names):
        ...
        first_param = next(gp.parameters())
        ...
        shap_vals = explainer.shap_values(X_eval, nsamples="auto")
        shap_matrix = _coerce_shap_to_2d(shap_vals)
        ...
        if np.allclose(mean_abs, 0.0):
            print(
                f"[plot_shap] SHAP values are all ~0 for {obj_name}. "
                "Model may be locally flat or SHAP could not resolve feature effects."
            )
            ax.text(
                0.5,
                0.5,
                "All SHAP magnitudes ~ 0",
