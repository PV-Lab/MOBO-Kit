...
class MixedMCMultiOutputObjective(MCMultiOutputObjective):
    """
    A custom BoTorch objective that handles maximizing, minimizing, 
    and matching target values all at once.
    """
    def __init__(self, directions):
        """
        Args:
            directions (list): A list defining the goal for each objective. 
                               e.g., ['max', 'min', ('match', 50.0)]
        """
        super().__init__()
    ...
    def forward(self, samples, X=None):
        """
        Applies transformations to the posterior samples so BoTorch 
        can maximize the resulting tensor for all objectives.
        """
        # Clone to prevent modifying the original tensor in-place
        Y_transformed = samples.clone()
        
        for i, direction in enumerate(self.directions):
            if direction == 'max':
                pass  # BoTorch natively maximizes; do nothing
            
            elif direction == 'min':
                Y_transformed[..., i] = -samples[..., i]
                
            elif isinstance(direction, tuple) and direction[0] == 'match':
                target_value = direction[1]
                # Maximize the negative distance to the target (pushes it to 0)
                Y_transformed[..., i] = -torch.abs(samples[..., i] - target_value)
